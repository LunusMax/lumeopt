...
\begin{figure}[hbt]
\centering
\caption{Pipeline CRISP-DM}\label{fig:crispdm_pipeline}
...
Na forma mais geral, a RL pode ser expressa conforme a Equação~\ref{eq:regressao_linear}:
...
\begin{figure}[hbt]
\centering
\caption{AD para Iris}\label{fig2_1}
\includegraphics[width=\textwidth]{imagens/ad_01.png}
\fonte{\cite{Geron2019})}
\end{figure}
...
Formalmente, a cada iteração $m$, busca-se adicionar um novo modelo $h_m(x)$ de forma que o modelo final seja definido conforme a Equação~\ref{eq:gradient_boosting}:
...
em que $\gamma_m$ representa o passo ótimo na direção do novo preditor.
...
O \textit{Multilayer Perceptron} (MLP) é uma função matemática parametrizada que 
aprende a melhor forma de transformar um vetor de entrada $\mathbf{x}$ em uma 
saída $\mathbf{y}$. Assim, o MLP pode ser entendido como uma composição de várias 
funções aplicadas em sequência, formando um modelo de múltiplas camadas \cite{Goodfellow2016}.

A arquitetura do MLP segue uma estrutura do tipo entrada → camadas ocultas → saída, 
utilizando uma função de ativação $g(\cdot)$ para introduzir não linearidade em 
cada camada. Essa relação pode ser expressa conforme a Equação~\ref{eq:mlp_camadas}:

\begin{equation}
h^{(l)} = g\!\left(W^{(l)}h^{(l-1)} + b^{(l)}\right)
\label{eq:mlp_camadas}
\end{equation}

em que $W^{(l)}$ e $b^{(l)}$ representam, respectivamente, a matriz de pesos 
e o vetor de vieses da camada $l$ \cite{Goodfellow2016}.

A introdução da função não linear $g(\cdot)$ é essencial, pois permite que 
o MLP aprenda relações complexas entre as variáveis de entrada e saída. Na ausência 
dessa não linearidade, o modelo se reduz a uma combinação linear das entradas, tornando-se 
incapaz de representar funções não lineares \cite{Goodfellow2016}.
...
Um dos problemas clássicos da otimização combinatória é o Problema da 
Mochila (\textit{Knapsack Problem}), amplamente utilizado como modelo abstrato 
para representar situações de seleção de recursos sob restrições limitadas \cite{Cormen2009}. 
Nesse problema, considera-se um conjunto de $n$ itens, no qual cada item $i$ possui um 
valor associado $v_i$ e um peso $w_i$. O objetivo consiste em selecionar um subconjunto 
desses itens de modo a maximizar o valor total transportado, sem que o peso total 
ultrapasse a capacidade máxima $W$ da mochila.

Na formulação conhecida como \textit{0-1 Knapsack Problem}, cada item pode ser 
selecionado integralmente ou não selecionado, caracterizando uma decisão binária para 
cada elemento do conjunto. Dessa forma, o problema pode ser formulado matematicamente 
conforme as Equações~\ref{eq:knapsack_obj}--\ref{eq:knapsack_binaria}:
...
\label{eq:knapsack_restricao}
\end{equation}

\begin{equation}
x_i \in \{0,1\}, \quad i = 1,2,\dots,n
\label{eq:knapsack_binaria}
\end{equation}
