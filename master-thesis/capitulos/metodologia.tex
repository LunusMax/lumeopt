...
Essa biblioteca é amplamente utilizada para extração e manipulação de dados a partir de páginas HTML e XML \cite{beautifulsoup}. 

O site utilizado para extração foi o FBref, uma ampla \textit{database} de estatísticas de futebol que contém dados detalhados de jogadores e equipes de diferentes 
competições ao redor do mundo. O FBref se destaca por disponibilizar estatísticas avançadas, como \textit{Expected Goals (XG)}, pressões, passes progressivos e 
estatísticas defensivas, tornando-se uma fonte valiosa para análises preditivas e modelagem estatística no futebol.

Com o intuito de utilizar mais dados que \textit{Baboota \& Kaur} (2018) e \textit{Joseph, Fenton \& Neil} (2006) e evitar \textit{overfitting}, para este estudo foram 
requisitados os dados de todos os jogadores da \textit{Serie A} italiana e \textit{Premier League} que atuaram ao longo de cinco 
temporadas (2019-2020, 2020-2021, 2021-2022, 2022-2023, 2023-2024), além da tabela de resultados de todos os jogos disputados nesse mesmo período. 
Os dados obtidos foram armazenados no formato csv, sendo organizados em arquivos distintos para cada time e temporada. Dessa forma, cada arquivo contém as 
estatísticas individuais dos jogadores de um determinado clube em uma temporada específica.

\section{Pré-processamento dos Dados}

\subsection{Base de Dados dos Jogadores}

As tabelas extraídas do site FBref para todos os times e temporadas foram estruturadas de forma idêntica, contendo 47 atributos relacionados a idade, posição, 
minutos jogados, bem como estatísticas ofensivas (gols, assistências, passes decisivos etc.) e defensivas (bloqueios, desarmes, erros graves etc.) de cada jogador. 
Esses dados foram carregados em diferentes \textit{dataframes}, um para cada liga considerada, e posteriormente organizados por jogador, incluindo time, temporada e 
respectivos indicadores. Ressalta-se que um mesmo atleta pode aparecer em diferentes temporadas e clubes dentro da organização adotada.

Os \textit{dataframes} correspondentes a cada liga foram posteriormente concatenados em um único conjunto, englobando todos os jogadores, o qual foi armazenado para as 
etapas seguintes da análise.

Com o intuito de evitar \textit{overfitting} e melhorar a eficiência computacional, foi aplicada uma técnica de redução de dimensionalidade, 
...
Com base nessa análise, serão realizados testes com diferentes quantidades de componentes, garantindo que sempre serão utilizados no mínimo 20 componentes, 
