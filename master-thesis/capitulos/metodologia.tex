...
\subsection{Algoritmo Guloso para Substituição Iterativa de Jogadores}

Com o objetivo de explorar possíveis melhorias na composição do elenco, foi implementado um procedimento guloso baseado em substituições iterativas de jogadores.

O método parte do elenco atual da equipe e realiza um número pré-definido de iterações. Em cada iteração, identifica-se inicialmente o jogador com menor 
minutagem na temporada considerada dentro do elenco analisado. Esse atleta é selecionado como candidato à remoção, assumindo-se que sua saída tende a causar 
menor impacto na estrutura do time.

Em seguida, considera-se um conjunto de jogadores candidatos à contratação. Para cada candidato, constrói-se temporariamente um novo elenco no qual o 
jogador selecionado para remoção é substituído pelo novo atleta, mantendo-se constante a cardinalidade total do grupo. Cada uma dessas configurações é então avaliada 
por meio da função objetivo definida neste trabalho, implementada como o modelo de aprendizado de máquina responsável por estimar a pontuação esperada da equipe na temporada.

Após avaliar todas as alternativas possíveis, seleciona-se o jogador cuja inclusão resulta no maior valor previsto da função objetivo. Essa substituição é então 
aplicada de forma definitiva ao elenco, e o processo é repetido na iteração seguinte.

Esse procedimento caracteriza uma estratégia gulosa, pois em cada etapa é escolhida a substituição que produz o maior ganho imediato na função objetivo, 
sem reconsiderar decisões tomadas em iterações anteriores. O pseudocódigo do algoritmo utilizado é apresentado no Algoritmo~\ref{alg:greedy_roster}.


\input{algoritmos/guloso}
...
Nesse contexto, foi adotado um algoritmo heurístico de busca local para explorar o espaço de possíveis combinações de jogadores candidatos à contratação.
A estratégia parte de uma solução inicial viável e realiza modificações incrementais na composição do elenco, buscando iterativamente soluções vizinhas
que resultem em maior valor da função objetivo.

Cada solução é representada por um conjunto de jogadores de cardinalidade fixa, correspondente ao número de contratações consideradas no processo de
otimização. A função objetivo utilizada para avaliar cada solução é dada pelo modelo de AM desenvolvido neste trabalho,
responsável por estimar a pontuação esperada da equipe ao final da temporada a partir das características estatísticas dos jogadores selecionados.

A geração da solução inicial ocorre por meio da amostragem aleatória de combinações de jogadores, sendo descartadas automaticamente aquelas que
violam a restrição orçamentária estabelecida. Uma vez obtida uma solução inicial viável, o algoritmo passa a explorar sua vizinhança por meio de
operações de substituição unitária (\textit{swap}), nas quais um jogador do conjunto atual é trocado por outro ainda não selecionado.

A cada substituição potencial, a nova configuração de elenco é avaliada pela função objetivo. Caso a nova solução apresente valor superior ao da
solução corrente, a substituição é aceita e o processo continua a partir da nova configuração. O critério de aceitação adotado é, portanto,
de melhoria estrita, caracterizando uma estratégia do tipo \textit{first improvement}.

Além disso, a restrição de orçamento é tratada diretamente durante a avaliação das soluções: combinações que excedem o limite estabelecido
são automaticamente descartadas pelo algoritmo. Dessa forma, garante-se que apenas soluções factíveis sejam consideradas durante o processo
de busca.

O algoritmo é encerrado quando nenhuma substituição viável produz melhoria adicional na função objetivo, indicando a convergência para um
ótimo local no espaço de soluções consideradas. A restrição orçamentária é tratada como condição de factibilidade durante a avaliação das
soluções vizinhas, de modo que configurações que excedem o limite estabelecido são simplesmente descartadas durante o processo de busca.
O pseudocódigo do procedimento de busca local implementado neste trabalho é apresentado no Algoritmo~\ref{alg:busca_local_budget}.

\input{algoritmos/busca_local}